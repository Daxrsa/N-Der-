...
namespace NDereAPI.Controllers
{
    ...
    public class KlientiController : ControllerBase
    {
        ...
        {
            var klienti = await dataContext.Klientis.FindAsync(id);
        ...
        {
            dataContext.Klientis.Add(klient);
        ...
        {
            var dbKlienti = await dataContext.Klientis.FindAsync(request.KlientiId);
            if (dbKlienti == null)
        ...
        {
            var dbKlienti = await dataContext.Klientis.FindAsync(id);
            if (dbKlienti == null)
                return BadRequest("Client not found!");

            dataContext.Klientis.Remove(dbKlienti);
            await dataContext.SaveChangesAsync();
